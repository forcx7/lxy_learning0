...
from GRL_Library.agent import DoubleDQN_agent  # 导入编写的GRL_agent
...
def Create_DQN(num_HVs, num_AVs, param):
        ...
        # replay_buffer_0 = prioritized_replay_buffer.PrioritizedReplayBuffer(capacity=2 ** 15,
        #                                                                     alpha=0.6,
        #                                                                     beta=0.4,
        #                                                                     beta_step=0.0001,
        #                                                                     epsilon=1e-5)
        ...
        explorer_0 = explorer.LinearDecayEpsilonGreedy(start_epsilon=0.9, end_epsilon=0.01, decay_step=20000)
        # 打印网络初始参数
        ...
        GRL_DQN = DoubleDQN_agent.DoubleDQN(
            ...
            update_interval=20,  # 当前网络更新步长间隔
            target_update_interval=5000,  # 目标网络更新步长间隔
            target_update_method='soft',  # 目标网络更新方法
            soft_update_tau=0.075111111111111111,  # 若soft_update，定义更新权重
            n_steps=1,  # multi-steps learning学习步长
            model_name="DoubleDQN_model"  # 模型命名
        )
